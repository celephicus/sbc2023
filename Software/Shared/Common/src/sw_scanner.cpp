...
void swScanInit(const sw_scan_def_t* defs, sw_scan_context_t* contexts, uint8_t count) {
	...
	fori (count) {
		...
		if (is_sw_active(&defs[i])) 
			eventPublish_P(&defs[i].event, EV_P8_SW_STARTUP);  // Event sent at startup when button found to be pressed. 
	}
		...
		if (sw_active && (!(contexts[i].pstate))) { 							// Is sw just ACTIVE...
			...
			if (contexts[i].pstate) {
				if (flags & mask) {												// Only post release if past action delay.
					eventPublish_P(&defs[i].event, EV_P8_SW_RELEASE); 
					flags &= ~mask;
				}
		...
		if (contexts[i].pstate) {												// Is sw currectly active...
			if (!(flags & mask)) {
				if (0 == contexts[i].action) {
					eventPublish_P(&defs[i].event, EV_P8_SW_CLICK);  			// Publish initial click event.
			...
			// Check hold timer...
				...
				if (SW_HOLD_TIME == contexts[i].hold)
					eventPublish_P(&defs[i].event, EV_P8_SW_HOLD); 
				else if (SW_LONG_HOLD_TIME == contexts[i].hold)
					eventPublish_P(&defs[i].event, EV_P8_SW_LONG_HOLD); 
				contexts[i].hold += 1;
			}
		}
	}
