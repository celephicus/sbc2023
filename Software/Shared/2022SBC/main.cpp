...
static bool console_cmds_user(char* cmd) {
  switch (console_hash(cmd)) {
    ...
    case /** ?PIN **/ 0xa9ad: 
		consolePrint(CFMT_D, (console_cell_t)digitalRead(consoleStackPop()));
        break;
    case /** PMODE **/ 0x48d6: {
        const uint8_t pin = (uint8_t)consoleStackPop();
...
#if CFG_DRIVER_BUILD == CFG_DRIVER_BUILD_SARGOOD
	{ REGS_FLAGS_MASK_FAULT_RELAY|REGS_FLAGS_MASK_FAULT_SENSOR_0|REGS_FLAGS_MASK_FAULT_SENSOR_1,	DRIVER_LED_PATTERN_NO_COMMS },
	{ REGS_FLAGS_MASK_DC_LOW,										DRIVER_LED_PATTERN_DC_LOW },
#endif
