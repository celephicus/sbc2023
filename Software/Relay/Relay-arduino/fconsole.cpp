...
uint8_t _FConsole::s_flags;

void _FConsole::begin(console_recogniser_func r_user, Stream& s, uint8_t flags) {
...
void _FConsole::prompt() {
	if (!(_FConsole::s_flags && _FConsole::F_NO_PROMPT))
...
void _FConsole::service() {
	if (FConsole.s_stream && (FConsole.s_stream->available())) {			// If a character is available...
		...
		if (rc >= CONSOLE_RC_OK) {								// On newline...
			if (!(_FConsole::s_flags && _FConsole::F_NO_ECHO)) {
				consolePrint(CONSOLE_PRINT_STR, (console_cell_t)consoleAcceptBuffer());	// Echo input line back to terminal.
				consolePrint(CONSOLE_PRINT_STR_P, (console_cell_t)PSTR("->")); 			// Seperator string for output.
			}
