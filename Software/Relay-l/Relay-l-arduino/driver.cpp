...
// Blinky LED.
static sequencer_context_t f_led_seq;
typedef struct {        
	sequencer_header_t hdr;
    uint8_t colour;
} led_pattern_def_t;

static void led_action_func(const sequencer_header_t* hdr) {
   const led_pattern_def_t* def = (const led_pattern_def_t*)hdr;
   gpioLedWrite((NULL != def) ? !!pgm_read_byte(&def->colour) : false);
}
void led_set_pattern(const led_pattern_def_t* def) { 
...
void driverSetLedPattern(uint8_t p) {
	if (p >= ELEMENT_COUNT(LED_PATTERNS))
		p = 0;
